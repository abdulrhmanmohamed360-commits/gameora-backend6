...
import { sendDisputeMessage } from "../lib/disputes";
import {
  parsePageParams,
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      /*
       * النزاع لازم يوصل للإدارة: بنفتح محادثة خاصة مع كل طرف (المشتري والبائع)
       * عشان يكتبوا للإدارة تفاصيلهم وأدلتهم، والإدارة تقدر تراسلهم من لوحة التحكم.
       * لو الخطوة دي فشلت النزاع نفسه يفضل مفتوح (والطلب ظاهر في لوحة الإدارة).
       */
      try {
        const disputed = result as FirebaseFirestore.DocumentData;
        const shortId = req.params.id.slice(0, 8);

        await sendDisputeMessage(
          req.params.id,
          disputed.buyerId,
          `استلمنا نزاعك على الطلب #${shortId}. اكتب هنا تفاصيل المشكلة وأي أدلة، وسنراجعها في أقرب وقت. لن يتم تحويل المبلغ للبائع قبل حل النزاع.`,
          "buyer"
        );
        ...
        await sendDisputeMessage(
          req.params.id,
          disputed.sellerId,
          `فتح المشتري نزاعًا على الطلب #${shortId}. اكتب هنا ردك وأي أدلة تدعم موقفك، وسنراجع الطرفين. لن يتم تحويل المبلغ قبل حل النزاع.`,
          "seller"
        );
      } catch (threadErr) {
        console.error("dispute threads failed", threadErr);
      }
